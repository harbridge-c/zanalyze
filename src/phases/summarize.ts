...
export const create = async (config: Config): Promise<[SummarizeAggregatorNode, SummarizePhaseNode]> => {
    ...
    const aggregate = async (input: PhaseInput, context: Context): Promise<Readonly<AggregationResult<PhaseOutput>>> => {
        ...
        if (output.events && output.people && output.classifications) {
            result = {
                status: 'Ready',
                output: output as PhaseOutput,
            }
        }
    ...
    const aggregatorNode = createAggregatorNode(
        SUMMARIZE_AGGREGATOR_NODE_NAME,
        aggregator,
        {
            next: [createConnection('toSummarize', SUMMARIZE_PHASE_NODE_NAME)],
        }
    );

    return [aggregatorNode, summarizePhaseNode];
}
