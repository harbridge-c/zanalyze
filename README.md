...
Dealing with email content for analysis can be a frustrating experience. Raw EML files are often cluttered with signatures, disclaimers, lengthy reply chains, and complex formatting – a lot of "nonsense" that gets in the way of understanding the core message. Crudzap is designed to tackle this challenge head-on. It's a tool built to proactively clean and simplify the content extracted from EML files, transforming them into a more manageable and analysis-ready format. By stripping away the unnecessary noise, Crudzap helps you focus on the valuable information within your emails, making the entire experience of working with email data much more efficient and insightful.

## Getting Started

To get started with Crudzap quickly, you can use `dlx` (part of Node.js/npm) to run it without needing to clone the repository or install it globally. You will also need to ensure your OpenAI API key is available as an environment variable.

1.  **Set your OpenAI API Key:**
    Crudzap uses OpenAI models for its classification and potentially other AI-driven tasks. You need to have an OpenAI API key and make it available to the application. The recommended way is to set the `OPENAI_API_KEY` environment variable:

    ```bash
    export OPENAI_API_KEY="your_openai_api_key_here"
    ```

    Also note that crudzap will attempt to load environment files from a .env file with dotenv.

2.  **Run Crudzap with `dlx`:**
    Open your terminal and use the following command structure. Replace `your-package-name` with the actual npm package name for Crudzap once it's published. You'll also need to specify your input and output directories.
...
This should give users a straightforward way to try out Crudzap and then delve into more advanced configurations as needed.


## Assumptions and Workflow

Crudzap is designed to fit into a broader email processing pipeline and makes a few key assumptions about how you'll use it:

1.  **Pre-existing EML Files:** Crudzap expects that you have already exported your emails from their source (e.g., Gmail, Outlook) into a directory structure containing individual EML files. Tools like [`@vortiq/gmlift`](https://www.npmjs.com/package/@vortiq/gmlift) (for Gmail) can be used for this initial export step.

2.  **Input Directory Structure:** While Crudzap can process EMLs from a flat directory, it works best with a structured input, particularly when using date-based processing. The default input (and output) structure is `"month"`. This means Crudzap expects your EML files to be organized into subdirectories by year and then by month. For example:
...
It's important to view Crudzap as a specialized tool focused on the initial cleaning, simplification, and classification of email content. It's designed to be a crucial first step in a more comprehensive data processing and analysis pipeline. The structured and cleaned output from Crudzap is intended to be consumed by other tools or processes that might perform tasks like advanced analytics, data visualization, or integration into knowledge bases.


## How Emails are Processed in Crudzap

The email processing system in Crudzap works in a sequence of steps to understand and organize your emails. Each step builds upon the last, ensuring emails are handled efficiently and categorized correctly. Here's what happens to an email as it goes through the system:
...
Crudzap's behavior can be customized through a `config.yaml` file and/or command-line arguments. Command-line arguments will always override values specified in the configuration file, which in turn override the application defaults.
